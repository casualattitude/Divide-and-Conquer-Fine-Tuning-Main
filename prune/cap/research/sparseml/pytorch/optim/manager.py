...
class RecipeManagerStepWrapper(object):
    ...
    def __init__(
        self,
        wrap: Any,#wrap：要包装的对象，通常是优化器。
        #optimizer：训练过程中使用的优化器
        optimizer: Optimizer,
        module: Module,#训练过程中使用的模型
        manager: Any,
        epoch: float,
        steps_per_epoch: int,
    ):
        if not isinstance(manager, ScheduledModifierManager):#检查传入的管理器是否是 ScheduledModifierManager 类的实例
            raise ValueError(
                "manager must be an instance of a ScheduledModifierManager"
            )

        if not hasattr(wrap, "step"):#检查要包装的对象是否具有 step 函数
            raise ValueError("wrapped must have a step function")

        if steps_per_epoch <= 0:#检查每个 epoch 中的步数是否大于 0
